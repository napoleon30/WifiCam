 ...
 * 
 */
public class ControlMsg6 {
	
	private static final int MAX = 0xff;
	private static final int MID = 0x7F;
	
	private static ControlMsg6 sControlMsg6;
	 ...
	 * @return
	 */
	public static ControlMsg6 getInstance() {
		synchronized (ControlMsg.class) {
			if (sControlMsg6 == null) {
				sControlMsg6 = new ControlMsg6();
			}
			return sControlMsg6;
		}
	}
...
//		}
		int tmp = 1;
		if(PlayActivity.radio_checked == 1){
			tmp= (int) (MID + f/8 * MID);
		}else if(PlayActivity.radio_checked == 2){
			tmp= (int) (MID + f/4 * MID);
		}else if(PlayActivity.radio_checked == 3){
			tmp= (int) (MID + f/2 * MID);
		}else if(PlayActivity.radio_checked == 4){
			tmp= (int) (MID + f * MID);
		}
		if(tmp > MAX) {
			tmp = MAX;
	...
	static int getCtlData2(float f) {
//		int mid = MID + trim;
//		if(f > 0) {
//			tmp = (int) (mid + f * (MAX - mid));
//		} else {
